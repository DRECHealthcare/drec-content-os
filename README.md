...
- Use the Doctor Send Queue inside Assets to copy the full doctor message, paste-back template, all doctor-review messages, single asset requests, and reply templates without touching scheduling, publishing, or Meta
...
- Use the Assets Next Asset Review card to identify the first pending asset, copy its safety review note, and jump to its controls without approving or queueing it automatically
...
- Use the First Cycle Handoff Pack to move from safe rewrites through human approval, doctor reply inbox, production reply inbox, pre-schedule checks, manual handoff, and metrics without bypassing safety gates
...
- Download the Production Handoff Bridge to copy one send-ready design/media message and matching paste-back production reply template after doctor approval
...
- Download the Production Reply Inbox Pack to collect designer media/design URLs in the exact paste-back format before previewing and importing production replies
...
- Download a protected Daily Ops Checklist with morning checks, today's priority, current-cycle action links, ready-to-publish items, blockers, learning prompts, and closeout steps
...
- View the live External Setup Board on the dashboard and download its CSV to track doctor approval, Supabase service-role, GitHub scheduler, Meta credential, Meta permission, and live-switch evidence in one place
...
- Download a protected operator pack with readiness status, current-cycle action links, setup checklist, publishing handoff, and weekly report in one Markdown file
...
- Use Suggested Learning Weights in Learning to turn measured outcome signals into reversible planning weights without approving content or changing publishing settings
...
- Download a Scheduler Health Pack from Meta Setup to diagnose missing or stale GitHub dry-run scheduler heartbeats without recording fake heartbeat evidence
- Download a Scheduler Recovery Pack from Meta Setup with GitHub Actions links, secret/variable checks, manual rerun steps, and verification commands for stale scheduler heartbeats
...
- Download, preview, and import a Schedule Worksheet CSV to batch-fill planned slots for already review-approved queue items without publishing or sending Meta requests
