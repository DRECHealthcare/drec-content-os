...
const checks = [
  ...
  },
  {
    ...
    validate: async (res) => {
      const text = await res.text();
      return text.includes("DREC") && text.includes("workflow-next") && text.includes("launch-count") && text.includes("token-input") && text.includes("copy-test-path") && text.includes("run-risk-audit") && text.includes("download-test-run-tracker") && text.includes("download-manual-cycle-qa") && text.includes("download-snapshot") && text.includes("download-operator-pack") && text.includes("Ready Assets") && text.includes("Learning Loop") && text.includes("Security Gate") && text.includes("Automation Gate") && text.includes("Record Published") && text.includes("Save & Roll Up") && text.includes("Use Topics In Weekly Plan") && text.includes("download-plan-csv") && text.includes("download-weekly-report") && text.includes("download-learning-snapshot") && text.includes("download-metrics-template") && text.includes("preview-metrics-csv") && text.includes("import-metrics-csv") && text.includes("metrics-import-preview") && text.includes("save-all-assets") && text.includes("archive-drafted-briefs") && text.includes("approve-clear-assets") && text.includes("queue-ready-assets") && text.includes("download-creative-pack") && text.includes("download-asset-review") && text.includes("download-asset-worklist") && text.includes("download-review-queue") && text.includes("download-review-log") && text.includes("download-run-sheet") && text.includes("download-calendar") && text.includes("download-schedule-csv") && text.includes("schedule-approved-items") && text.includes("kb-context") && text.includes("copy-meta-setup") && text.includes("dry-run-meta-publishing");
    },
  },
  {
    name: "Web script",
    ...
    validate: async (res) => {
      const text = await res.text();
      return text.includes("/workflow/status") && text.includes("/operations/launch-readiness") && text.includes("/operations/test-run-checklist") && text.includes("/operations/test-run-tracker.md") && text.includes("/operations/manual-cycle-qa.md") && text.includes("data-test-run-screen") && text.includes("/operations/risk-audit") && text.includes("/operations/snapshot.csv") && text.includes("/operations/creative-pack.md") && text.includes("/operations/asset-review.csv") && text.includes("/operations/asset-review-worklist.md") && text.includes("/operations/review-log.md") && text.includes("/operations/review-queue.csv") && text.includes("/operations/learning-snapshot.csv") && text.includes("/operations/publishing-run-sheet.md") && text.includes("/operations/operator-pack.md") && text.includes("Existing queue item opened") && text.includes("function testPathText()") && text.includes("saveAccessTokenFromPanel") && text.includes("countByStatus") && text.includes("security-count") && text.includes("automation-count") && text.includes("Record Published: After manual posting") && text.includes("Save & Roll Up: Add metrics") && text.includes("Use Topics: Send learning recommendations") && text.includes("loadLearningTopicsIntoPlan") && text.includes("/briefs/plan.csv") && text.includes("Outcome Insights") && text.includes("outcome_insights") && text.includes("needsReviewQueue") && text.includes("handoff_blockers") && text.includes("schedule-next") && text.includes("/publish-queue/calendar.ics") && text.includes("/publish-queue/schedule.csv") && text.includes("load-published-post") && text.includes("save-rollup-metric") && text.includes("renderMetricsImportPreview") && text.includes("/metrics/import-csv") && text.includes("/kb/context") && text.includes("Active Knowledge Context") && text.includes("/briefs/draft-assets") && text.includes("/briefs/archive-drafted") && text.includes("/assets/queue-ready") && text.includes("/publish-queue/schedule-approved") && text.includes("data-handoff-published") && text.includes("/meta/setup-checklist") && text.includes("meta-setup-commands") && text.includes("GitHub Scheduler Setup") && text.includes("schedulerHeartbeat.detail") && text.includes("scheduler_setup") && text.includes("/jobs/meta-publishing?dry_run=true&channel=all");
    },
  },
];
